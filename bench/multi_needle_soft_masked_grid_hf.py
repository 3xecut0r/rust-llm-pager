...
POLICIES = ["full_context", "recent_only", "sinks_recent", "heavy_hitter", "sinks_heavy_hitter"]

OUT_PATH = Path("bench/multi_needle_soft_masked_grid_results.csv")


def metrics_to_dict(m):
    """Flatten a pager metrics object into a plain dict, including the VRAM attention ratio."""
    return {
        "tokens": m.tokens,
        ...
        "vram_attention_ratio": (m.attention_mass_vram / m.attention_mass_total if m.attention_mass_total else 0),
    }


def token_attention_to_blocks(attn_to_keys: torch.Tensor, tokens_per_block: int) -> list[float]:
    """Sum per-token attention into per-block attention totals."""
    seq_len = attn_to_keys.shape[0]

    out = [0.0 for _ in range((seq_len + tokens_per_block - 1) // tokens_per_block)]
...
def extract_last_query_traces(outputs) -> list[tuple[int, list[float]]]:
    """Return each layer's per-block attention trace for the last query token."""
    ...
    for layer_idx, layer_attn in enumerate(attentions):
        ...
        block_attn = token_attention_to_blocks(attn_to_keys, TOKENS_PER_BLOCK)
...
def build_tier_attention_bias(*, base_attention_mask: torch.Tensor, tiers: list[int]) -> torch.Tensor:
    """Build an additive attention bias that penalizes RAM/SSD-tier keys instead of hiding them."""
    ...
    bias = torch.zeros((batch_size, 1, seq_len, seq_len), device=device, dtype=dtype)

    key_penalties = torch.zeros((seq_len,), device=device, dtype=dtype)
    ...
    if padding_mask.any():
        bias = bias.masked_fill(padding_mask.view(batch_size, 1, 1, seq_len), -1e4)

    # Safety anchors.
    # Token 0 and current token should not be hard-masked.
    bias[:, :, :, 0] = torch.maximum(bias[:, :, :, 0], torch.tensor(0.0, device=device, dtype=dtype))
    bias[:, :, -1, -1] = 0.0

    return bias


def score_expected_answer(*, model, tokenizer, prompt: str, policy: str, expected_answer: str, needle_id: str) -> dict:
    """Teacher-force the expected answer under a policy's soft-masked attention and score it."""
    device = next(model.parameters()).device

    if device.type == "cuda":
        torch.cuda.empty_cache()

    prompt_encoded = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=MAX_LENGTH)
    ...
    answer_ids = tokenizer(expected_answer_with_space, return_tensors="pt", add_special_tokens=False)["input_ids"][
        0
    ].to(device)
    ...
    with torch.inference_mode():
        for target_id in answer_ids:
            ...
            if policy == "full_context":
                ...
                trace_outputs = model(
                    input_ids=input_ids, attention_mask=attention_mask, output_attentions=True, use_cache=False
                )

                for layer_idx, block_attn in extract_last_query_traces(trace_outputs):
                    p.on_step(current_query_idx, layer_idx, block_attn)

                scoring_attention_mask = build_tier_attention_bias(base_attention_mask=attention_mask, tiers=p.tiers())
            ...
            score_outputs = model(
                input_ids=input_ids, attention_mask=scoring_attention_mask, output_attentions=False, use_cache=False
            )
            ...
            logprobs.append(float(log_probs[0, int(target_id.item())].item()))
            predicted_ids.append(int(torch.argmax(logits, dim=-1).item()))

            # Teacher forcing: append correct token, not predicted token.
            input_ids = torch.cat([input_ids, target_id.view(1, 1)], dim=-1)
    ...
    return {
        ...
        "predicted_text_greedy_per_step": tokenizer.decode(predicted_ids, skip_special_tokens=True),
        "elapsed_sec": elapsed,
        "swap_total_gb": (metrics["swap_vram_ram"] + metrics["swap_ram_ssd"]) / 1_000_000_000,
        **metrics,
    }


def main():
    """Run the full needle x ram-penalty x ssd-penalty x policy grid and write results to CSV."""
    ...
    for needle in NEEDLES:
        ...
        decoded_prompt = tokenizer.decode(tokenized["input_ids"][0], skip_special_tokens=True)
        ...
        for ram_penalty in RAM_ATTENTION_PENALTIES:
            for ssd_penalty in SSD_ATTENTION_PENALTIES:
                ...
                print(f"\nPenalty setting: RAM={RAM_ATTENTION_PENALTY}, SSD={SSD_ATTENTION_PENALTY}")
