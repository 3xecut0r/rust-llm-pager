...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Persistent CPU KV paging policy comparison"
    )
    parser.add_argument(
        "--tokens",
        type=int,
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="suppress per-policy debug output, only print the final table",
    )
...
def run_policy(
    ...
    store, num_layers, num_blocks, tail_past, _ = build_initial_store(
        current_past
    )
    ...
    return PolicyRunResult(
        ...
        mean_attention_in_gpu=sum(attention_in_gpu_values)
                              / len(attention_in_gpu_values),
...
def write_results_csv(results: list[PolicyRunResult]) -> Path:
    ...
    with out_path.open("w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=list(PolicyRunResult.__dataclass_fields__.keys()),
        )
        writer.writeheader()
...
def write_results_markdown(
    ...
    ranked = sorted(
        results,
        key=lambda item: item.mean_attention_in_gpu,
        reverse=True,
    )
