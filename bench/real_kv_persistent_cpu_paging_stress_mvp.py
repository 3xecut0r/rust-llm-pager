...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Persistent CPU KV paging MVP stress test"
    )
    parser.add_argument(
        "--tokens",
        type=int,
    ...
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="suppress per-step debug output, only print the final summary",
    )
...
def reload_all_blocks_for_forward(
        *,
        store: KVBlockStore,
        device: torch.device,
        num_blocks: int,
...
def main() -> None:
    ...
    store, num_layers, num_blocks, tail_past, full_tokens = build_initial_store(
        current_past,
        verbose=not quiet,
    )
    ...
    for step in range(1, generate_tokens + 1):
        reload_bytes, reload_copies = reload_all_blocks_for_forward(
            store=store,
            device=device,
        ...
        (
            ...
            full_tokens,
            added_block_ids,
        ) = append_new_full_blocks_if_needed(
            store=store,
        ...
        step_gpu_to_cpu_bytes = (
                summary_after_placement["gpu_to_cpu_bytes"]
                - summary_before_placement["gpu_to_cpu_bytes"]
        )
        step_gpu_to_cpu_copies = (
                summary_after_placement["gpu_to_cpu_copies"]
                - summary_before_placement["gpu_to_cpu_copies"]
        )

        total_gpu_to_cpu_bytes += step_gpu_to_cpu_bytes
        total_cpu_to_gpu_bytes += reload_bytes
        total_gpu_to_cpu_copies += step_gpu_to_cpu_copies
        ...
        if not quiet:
            ...
            print(
                "resident_gpu_mb:",
                f"{summary_after_placement['resident_gpu_bytes'] / 1_000_000:.2f}",
            )
            print(
                "resident_cpu_mb:",
                f"{summary_after_placement['resident_cpu_bytes'] / 1_000_000:.2f}",
            )
    ...
    print(
        ...
        f"{sum(attention_in_gpu_values) / len(attention_in_gpu_values):.4f}",
    )
