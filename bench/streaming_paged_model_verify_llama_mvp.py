...
# Same verification as streaming_paged_model_verify_mvp.py, but on a Llama-family
# model instead of Qwen2 -- proves use_streaming_attention's RoPE dispatch
# (see _compute_rope in paged_model.py) actually works on the *other* architecture
# it claims to support, not just the one it was originally developed against.
...
def main() -> None:
    ...
    if all_ok:
        print(
            "\nOK: streaming PagedModel matches baseline and non-streaming PagedModel for all 4 policies on Llama too."
        )
    else:
