...
def trim_past_to_full_blocks(past_key_values, *, full_tokens: int) -> list[tuple[torch.Tensor, torch.Tensor]]:
    """Cut every layer's KV down to just the full-block portion, dropping any tail."""
    return [
...
def greedy_decode_from_cache(
    *, model, tokenizer, past_key_values, next_input_id: torch.Tensor, base_attention_mask: torch.Tensor, steps: int
) -> list[int]:
    """
    ...
    with torch.inference_mode():
        for _ in range(steps):
            outputs = model(
                input_ids=current_input_id, attention_mask=attention_mask, past_key_values=cache, use_cache=True
            )
...
def main() -> None:
    ...
    original_trimmed_past = trim_past_to_full_blocks(original_past, full_tokens=full_tokens)
    ...
    print("original_text:", repr(tokenizer.decode(original_generated)))
    print("reconstructed_text:", repr(tokenizer.decode(reconstructed_generated)))

    assert original_generated == reconstructed_generated

    print("\nOK: reconstructed real Qwen KV cache produces identical " "multi-token greedy generation.")
